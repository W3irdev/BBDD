...
CONSTRAINT chk1_demarcacion CHECK (DEMARCACION IN ('portero','otro')
);
...
CONSTRAINT CHK2_COMPETICION CHECK (UPPER(COMPETICION) IN ('COPA','LIGA'))
);
...
ALTER TABLE JUGADORES ADD CONSTRAINT CH_JUGADORES CHECK (INITCAP(NOMBRE) = NOMBRE); -- LA PRIMERA LETRA LA PONE EN MAYUSCULAS CON INITCAP
ALTER TABLE EQUIPOS ANDD CONSTRAINT CH_CODEQUIPO CHECK (REGEXP_LIKE(CODEQUIPO, '^[0-9{1}%'));
ALTER TABLE INCIDENCIAS MODIFY TIPO VARCHAR(20) NOT NULL;
ALTER TABLE EQUIPOS ADD GOLESMARCADOS inteGER(3);